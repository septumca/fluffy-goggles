...
use crate::{effect::{EffectsToPerform, GenericEffect}, token::{TokenContainer, Token, TokenWithValue}, damage::DmgRange, Actor};
...
impl Action for SloppyJab {
    ...
    fn perform(&self) -> EffectsToPerform {
        (
            vec![
                Box::new(GenericEffect::AddToken(TokenWithValue::new(Token::Unstability, 1))),
                Box::new(GenericEffect::RemoveToken(TokenWithValue::new(Token::Stamina, 1))),
            ],
            vec![
                Box::new(GenericEffect::AddToken(TokenWithValue::new(Token::Damage, self.damage.generate()))),
            ],
        )
    }
}
...
impl Action for RegainStance {
    ...
    fn perform(&self) -> EffectsToPerform {
        (
            vec![
                Box::new(GenericEffect::RemoveToken(TokenWithValue::new(Token::Unstability, u8::MAX))),
            ],
            vec![],
        )
    }
...
impl Action for Regeneration {
    ...
    fn perform(&self) -> EffectsToPerform {
        (
            vec![
                Box::new(GenericEffect::RemoveToken(TokenWithValue::new(Token::Damage, 1))),
            ],
            vec![],
        )
    }
